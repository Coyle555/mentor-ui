...
test('Render of the footer with a index out of total count', () => {
	const tree = renderer.create(<Footer currentIndex={1} totalRecords={20} />).toJSON();
	expect(tree).toMatchSnapshot();
});

test('Render of footer with next button enabled', () => {
	const tree = renderer.create(<Footer currentIndex={1} hasNext={true} totalRecords={20} />).toJSON();
	expect(tree).toMatchSnapshot();
});

test('Render of footer with previous button enabled', () => {
	const tree = renderer.create(<Footer currentIndex={1} hasPrevious={true} totalRecords={20} />).toJSON();
	expect(tree).toMatchSnapshot();
});
